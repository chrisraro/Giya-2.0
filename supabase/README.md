...
- **0056-0058 (task 2.2's three) are not present as files in THIS worktree.**
  ...
  what is actually live rather than implying only five migrations exist
  between 0055 and 0061.
- **One migration renamed twice: `0058` -> `0059` -> `0060`.** Task 2.5's
  `job_alert_state` table (authored as `0058_job_health_alerts.sql`, applied
  live under that exact name) collided on file number 0058 with task 2.2's
  deployment correction, authored concurrently in a separate checkout
  (`0058_balance_check_deployment_correction`) - renamed to `0059` to give a
  merged file set a defined replay order. Task 2.2's NEXT pass then also took
  `0059` (`0059_balance_check_trigger_privilege_fix`), so it was renamed again
  to `0060`, its final resting number. Only the FILENAME ever moved; the body
  is byte-identical to what ran, and the live ledger row still reads
  `0058_job_health_alerts` throughout - Supabase keys on the version
  timestamp, never the filename, so the database was never ambiguous. See
  `supabase/migrations/0060_job_health_alerts.sql`'s own top note for the full
  account - the same drift-gets-written-down rule 0011b and the 0042/0047
  incident both established.
