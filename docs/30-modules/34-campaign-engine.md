...
| → `paused` (T5) | Cache invalidation as above; live-eligibility Redis key deleted so `35` stops applying its rules immediately; if system-triggered (budget), enqueue in-app + push notification `kind='campaign_budget_exhausted'` to owner + managers; emit `campaign.paused` with `reason` |
...
|---|---|---|
| `max_total_points` | **Award time** — `35-points-engine.md` pipeline, before writing the ledger row | Running total = `select coalesce(sum(points),0) from points_transactions where campaign_id=$1 and points > 0` (cached in Redis `{env}:campaign:{id}:points_total`, re-derived on cache miss and by the nightly integrity job). If `total + pending_award > max_total_points`, the campaign's multiplier/bonus is skipped for this receipt (base points still awarded) and exhaustion fires |
...
| `per_customer_limit` | **Claim time** for reward-family (counts that consumer's non-cancelled `reward_claims`); **award time** for multiplier/bonus campaigns (counts that consumer's positive `points_transactions` with this `campaign_id`) | Note: `rewards.per_customer_limit` also exists per reward row; the stricter of the two applies |

**On exhaustion** of `max_total_points` or `max_redemptions`: the enforcing service calls the state machine with system actor → T5 `active → paused`, audit `campaign.paused` with `reason='budget_exhausted'`, notification `kind='campaign_budget_exhausted'` to owner + managers (push + in-app), analytics event `campaign.budget_exhausted`. `per_customer_limit` hitting for one consumer is *not* exhaustion — it returns `422 CAMPAIGN_LIMIT_REACHED` to that consumer only.
...
1. None required for the engine itself — lifecycle, scheduling, targeting, budget, and stacking operate entirely on existing columns; budget consumption totals are intentionally *derived* (ledger/claims sums + Redis cache), consistent with `../20-data/20-data-model.md` "balances are derived".
