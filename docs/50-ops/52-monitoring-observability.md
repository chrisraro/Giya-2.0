...
3. **Synthetic monitors (external):** uptime probes on `giya.ph`, `app.giya.ph`, `/api/v1/health` (returns dependency statuses: DB, Redis, QStash publish, OCR `/readyz`, storage) from ≥2 regions every minute; a weekly synthetic full scan→award transaction against production using a dedicated synthetic tenant (excluded from business metrics by `business_id` allowlist).
4. **Provider dashboards** (Vercel, Supabase, Upstash, Groq, Resend, GCP) are linked from the runbook index — used for capacity, not paged alerts, except where webhooked into the alert router.
