...
| `HF_TOKEN` | Hugging Face | VLM receipt transcription and the embeddings behind template retrieval | **SET and verified 2026-07-26** (account `Giya2026`). Used on two paths: `google/gemma-4-26B-A4B-it` for transcription (1.8s measured) and `all-MiniLM-L6-v2` for 384-dim embeddings. Rotate before production, it was pasted in chat. **`canPay: false`**, so VLM calls draw on free monthly credits from third-party providers and will throttle under real load; enable billing or pin a paid provider before launch. |
| `HF_VLM_MODEL` | Hugging Face | Which VLM transcribes receipts | `google/gemma-4-26B-A4B-it`, chosen by measurement. Do not switch to `gemma-4-31B-it`: it is a reasoning model that returns empty `content`. |
