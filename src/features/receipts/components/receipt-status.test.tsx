...
describe("the install prompt's trigger (doc 41 section 2)", () => {
  // "Trigger: after the first successful scan - when the user's first receipt
  // reaches `approved` (the moment of demonstrated value)."
  //
  // This screen is the publisher and knows nothing about installability: it
  // announces the moment, and src/components/pwa/install-prompt.tsx decides
  // whether anything is shown, subject to the budget in
  // src/features/pwa/install-prompt.ts. Splitting it that way is what lets the
  // trigger be asserted here without a service worker or a browser install.
  //
  // WHAT THIS PROVES: the event fires exactly when a receipt of theirs is
  // approved, once, and never for any other outcome. It does NOT prove a sheet
  // appears - a moment with no captured `beforeinstallprompt`, or a consumer
  // who has spent their three asks, is meant to be silent.

  function momentsDuring(body: () => void): number {
    let count = 0;
    const listener = () => {
      count += 1;
    };
    // The event NAME as a literal, deliberately: importing the constant the
    // component dispatches would make both sides of this agree by definition.
    window.addEventListener("giya:install-moment", listener);
    try {
      body();
    } finally {
      window.removeEventListener("giya:install-moment", listener);
    }
    return count;
  }

  it("CRITICAL: announces the moment when a receipt is already approved on arrival", () => {
    // The stub OCR provider settles inside one request, so the server's own
    // read can render `approved` on first paint and no Realtime event ever
    // arrives. A transition-only trigger would miss that path entirely, and it
    // is the common one.
    const moments = momentsDuring(() => {
      render(<ReceiptStatus receipt={baseReceipt({ status: "approved", pointsAwarded: 245 })} />);
    });

    expect(moments).toBe(1);
  });

  it("CRITICAL: announces the moment when the status flips to approved live", () => {
    mocks.fetchReceiptDetail.mockResolvedValue(
      baseReceipt({ status: "approved", pointsAwarded: 245 }),
    );
  ...
  it("CRITICAL: announces nothing for any other outcome", () => {
    for (const status of ["queued", "processing", "review", "rejected"] as const) {
      const moments = momentsDuring(() => {
        render(<ReceiptStatus receipt={baseReceipt({ status })} />);
      });
      expect(moments, status).toBe(0);
    }
  });

  it("CRITICAL: announces once, not once per re-render", () => {
    // An approved screen re-renders several times over: the points figure
    // lands a beat after the status flips, from a second fetch. Driven here
    // through `rerender` rather than through Realtime because that renders
    // unconditionally - a Realtime payload carrying no change can be dropped
    // before it reaches React, so it would not tell an effect keyed on the
    // status apart from one keyed on nothing.
    //
    // The failure this stops is expensive: the sheet costs one of three
    // lifetime asks each time it is shown, so an effect firing per render
    // would spend the whole budget on a single approved receipt.
    const approved = baseReceipt({ status: "approved", pointsAwarded: 245 });

    const moments = momentsDuring(() => {
      const { rerender } = render(<ReceiptStatus receipt={approved} />);
      rerender(<ReceiptStatus receipt={approved} />);
      rerender(<ReceiptStatus receipt={{ ...approved, pointsAwarded: 250 }} />);
    });
